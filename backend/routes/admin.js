...
const standardBannerRoutes = require('./admin/standardBanner');
...
// Used by AI image generator to fetch images from Stable Horde/R2 storage
router.post('/proxy-image', auth, adminAuth, async (req, res) => {
  try {
    const { url } = req.body;
    ...
    if (!isAllowed) {
      return res.status(400).json({
        error: 'Domain not allowed. Only Stable Horde and Cloudflare R2 URLs are permitted.'
      });
    }

    // Generate unique filename
    const ext = getExtensionFromUrl(url, '.png');
    const filename = generateUniqueFilename('ai-generated', ext);

    // Download the image to local storage
    const filepath = await downloadImage(url, filename, 'characters', {
      timeout: 60000 // 60 second timeout for AI-generated images
    });

    // Return the local URL path
    const urlPath = `/uploads/characters/${filename}`;
...
// ===========================================
// STANDARD BANNER MANAGEMENT (extracted to sub-router)
// ===========================================
router.use('/standard-banner', standardBannerRoutes);
