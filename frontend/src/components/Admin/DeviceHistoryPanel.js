...
const DeviceHistoryPanel = ({ userId, username }) => {
  ...
  const formatTimestamp = (ts) => ts ? new Date(ts).toLocaleString() : 'Unknown';
  ...
  if (loading) {
    return (
      <Container>
        <LoadingState>
          <FaSync className="spin" /> Loading device history...
        </LoadingState>
      </Container>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Summary>
        <SummaryItem>
          <SummaryLabel>Current Devices</SummaryLabel>
          <SummaryValue>{data.currentDevices?.length || 0}</SummaryValue>
        </SummaryItem>
        <SummaryItem>
          <SummaryLabel>Total Ever Seen</SummaryLabel>
      ...
      {data.deviceHistory?.length > 0 ? (
            ...
            {data.deviceHistory.map((device, index) => (
              ...
              >
                ...
                <DeviceInfo>
                  ...
                  <DeviceMeta>
                    <MetaItem>
                      <strong>First seen:</strong> {formatTimestamp(device.firstSeen)}
                    </MetaItem>
                    <MetaItem>
                      <strong>Last seen:</strong> {formatRelativeTime(device.lastSeen)}
                    </MetaItem>
                    <MetaItem>
                      <strong>Logins:</strong> {device.loginCount || 0}
                    </MetaItem>
                    {device.ipCount > 1 && (
                      <MetaItem $warning>
                        <FaGlobe /> {device.ipCount} different IPs
                      </MetaItem>
                    )}
                  </DeviceMeta>
                </DeviceInfo>
        ...
        <EmptyState>No device history available</EmptyState>
      )}

      {data.deviceEvents?.length > 0 && (
        <EventsSection>
          <EventsTitle>
            <FaExclamationTriangle /> Recent Device Events
          </EventsTitle>
          <EventsList>
