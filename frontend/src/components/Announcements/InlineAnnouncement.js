/**
 * InlineAnnouncement - Embedded announcements that scroll with page content
 *
 * Features:
 * - Inline positioning (part of document flow, scrolls with page)
 ...
 *
 * Use this component within page content where you want announcements
 * to appear inline rather than fixed at the top.
 *
 * @example
 * <InlineAnnouncement />
 */
...
const InlineAnnouncementItem = ({ announcement, onDismiss, onAcknowledge }) => {
...
InlineAnnouncementItem.propTypes = {
...
const InlineAnnouncement = ({ maxItems = 3 }) => {
  const { inlineAnnouncements, dismiss, acknowledge } = useAnnouncements();
  const [dismissedIds, setDismissedIds] = useState(() => {
    // Load dismissed IDs from localStorage
    try {
      const stored = localStorage.getItem('dismissedInlineAnnouncements');
  ...
  const visibleAnnouncements = useMemo(() => {
    return inlineAnnouncements
      .filter(a => !dismissedIds.has(a.id))
      .slice(0, maxItems);
  }, [inlineAnnouncements, dismissedIds, maxItems]);
  ...
  const handleDismiss = useCallback(async (id) => {
    ...
    setDismissedIds(prev => {
      ...
      next.add(id);
      // Persist to localStorage (separate from banner dismissals)
      localStorage.setItem('dismissedInlineAnnouncements', JSON.stringify([...next]));
      return next;
    });
...
InlineAnnouncement.propTypes = {
  /** Maximum number of inline announcements to show at once */
